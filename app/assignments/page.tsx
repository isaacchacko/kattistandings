...
const CACHE_KEY_ASSIGNMENTS = 'kattistandings_assignments';
...
// Helper functions for caching assignments
const getCachedAssignments = (): Assignment[] | null => {
  if (typeof window === 'undefined') return null;
  try {
    const cached = localStorage.getItem(CACHE_KEY_ASSIGNMENTS);
    if (!cached) return null;
    
    const parsed: CachedData<HomeData> = JSON.parse(cached);
    const now = Date.now();
    
    // Check if cache is still valid (less than 5 minutes old)
    if (now - parsed.timestamp < CACHE_DURATION_MS) {
      return parsed.data.assignments || null;
    }
    
    // Cache expired, remove it
    localStorage.removeItem(CACHE_KEY_ASSIGNMENTS);
    return null;
  } catch (err) {
    console.warn('Failed to read cached assignments:', err);
    return null;
  }
};

const setCachedAssignments = (data: HomeData): void => {
  if (typeof window === 'undefined') return;
  try {
    const cached: CachedData<HomeData> = {
      data,
      timestamp: Date.now(),
    };
    localStorage.setItem(CACHE_KEY_ASSIGNMENTS, JSON.stringify(cached));
  } catch (err) {
    console.warn('Failed to cache assignments:', err);
  }
};
...
export default function AssignmentsPage() {
  ...
  const fetchAssignments = async (skipCache = false) => {
    try {
